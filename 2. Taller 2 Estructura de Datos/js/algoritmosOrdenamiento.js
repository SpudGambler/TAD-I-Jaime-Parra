...
function bubbleUp(){
    for (let i = 0; i < selectionGroup.length; i++) {
       for (let j = 0; j < selectionGroup.length - 1 - i; j++) {
        (selectionGroup[j].order > selectionGroup[j + 1].order) ? [selectionGroup[j], selectionGroup[j + 1]] = [selectionGroup[j + 1], selectionGroup[j]] : '';
       }
    }
}
